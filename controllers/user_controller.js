    ...
    .then(
        function(err){
            if (err) {
                ...
                .save()
                .then( function(){
                    // crea la sesión para que el usuario acceda ya autenticado y redirige a path anterior
                    req.session.user = {id:user.id, username:user.username};
                    res.redirect(req.session.redir.toString()); //res.redirect('/');
                }); 
            }
        }
    ).catch(function(error){next(error)});
...
exports.update = function(req, res, next) {
  req.user.username  = req.body.login;
  ...
  .then(
    function(err){
      if (err) {
        ...
        .then( function(){ res.redirect(req.session.redir.toString());});
...
exports.destroy = function(req, res) {
  req.user.destroy().then( function() {
    // borra la sesión y redirige a path anterior
